...
import cv2
from model import net363 as network, utils

# Testing settings


parser = argparse.ArgumentParser()
parser.add_argument("--test_hr_folder", type=str, default=r'F:\datasets\SISR\Test\Set5_HR',
                    help='the folder of the target images')
parser.add_argument("--test_lr_folder", type=str, default=r'F:\datasets\SISR\Test\Set5_LR3',
                    help='the folder of the input images')

parser.add_argument("--output_folder", type=str, default=r'./results/x2/Set14')
# parser.add_argument("--checkpoint", type=str, default='./checkpoint_x4/weight36/1/epoch_1.pth',
#                     help='checkpoint folder to use')
parser.add_argument("--checkpoint", type=str, default=r'./checkpoint_x4/epoch_315.pth',
                    help='checkpoint folder to use')
...
model = network.HLFA(64,3,0.5)
# model = network.BSRN()
# model_dict = utils.load_state_dict(opt.checkpoint)
  # 315  339

psnr = []
# for checkpoint in range(1,405):
opt.checkpoint = r'E:\5.机器学习project\图像超分\模型设计\light_super_resolution\checkpoint_x3/epoch_2' + '.pth'
